...
/*const fetchQuoteOfTheDayAction = () => async dispatch => {
    const response = await fetchQuoteOfTheDay();

    console.log('response->>', response);
    dispatch({type: FETCH_QUOTE_OF_THE_DAY, payload: response});
};*/
/*const fetchQuoteOfTheDayAction = () => {
    console.log('method-->>>', fetchQuoteOfTheDay());
    return {
        type: FETCH_QUOTE_OF_THE_DAY,
        payload: {quote: fetchQuoteOfTheDay()},
    }
};*/
